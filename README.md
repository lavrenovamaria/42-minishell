...
## Cursor control using termcap library
...
Поле actions функции tcsetattr управляет способом внесения изменений. \
Есть три варианта:\
...
Это подпрограмма, которая устанавливает запись типа терминала, соответствующую имени, чтобы можно было выполнить запрос TermCap для записи. Как правило, значение, присвоенное имени, использует тип терминала, назначенный в качестве переменной среды TERM. В этом случае обычно присваивается значение NULL, поскольку указатель буфера, называемый bp, является игнорируемым аргументом.
